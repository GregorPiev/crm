...
$(document).on('click','a.getUncalledCustomers',function(){
       ...
       getUncalledCustomers(campaign);
});

$(document).on('click','a.getFailedAttempts',function(){
       var campaign = $(this).attr('data-campaign');
       getFailedAttempts(campaign);
});
...
function getUncalledCustomers(campaign){
	...
	var post_data = {'dpStart': $('#dpStart').val(),'dpEnd': $('#dpEnd').val(),'field': JSON.stringify({'value':campaign,'name':'campaign'})};
...
function getFailedAttempts(campaign){
	var url = location.protocol + '//' + location.host + '/'+globalBrandName+'/agenttools/customer_card';
	
	bootbox.dialog({
		title:   'Failed Attempts for '+campaign,
		message: '<div class="row">  ' +
                 '<div class="col-md-12"> ' +
...
function getLeadsByCampaigns() {
    $post_data = $('#range-form').serialize()+'&field=campaign';
    ...
    apiRequestBrand('getLeadsByField',$post_data,'#transactions_table_holder',true,capitalBrandName,function(data){
        ...
        $('#transactions_table').dataTable({
            ...
            "aaSorting": [[9, "desc"]],
            "aoColumns": [
                {"mData": "leadAggregateField", "sTitle": "Campaign"},
                ...
                {"mData": "percentCalled", "sTitle": "Called (%)", "sType": "numeric",
                 "mRender" : function( data, type, row ){
                 	 if(type ==='display')
                 	    return data+' <a class="btn btn-xs btn-blue btn-block getUncalledCustomers" data-campaign="'+row.leadAggregateField+'">View</a>';
                ...
                {"mData": "attemptCount", "sTitle": "Failed Attempts", "sType": "numeric",
                 "mRender": function(data,type, row){
                 	 if(type ==='display')
                 	    return data+' <a class="btn btn-xs btn-blue btn-block getFailedAttempts" data-campaign="'+row.leadAggregateField+'">View</a>';
...
function getAllLeads() {
    apiRequestBrand('getAllLeads', '', '#alltime_leads_holder',true,capitalBrandName, function (data) {
        $('#alltime_leads').dataTable({
            ...
            "aoColumns": [
                ...
                {"mData": "totalDepositsUSD", "sTitle": "Total Deposits ($)", "sType": "numeric"},
                {"mData": "totalDepositCancelledUSD", "sTitle": "Total Deposit Cancelled ($)", "sType": "numeric"},
                {"mData": "totalLeads", "sTitle": "Leads", "sType": "numeric"},
                {"mData": "totalCustomers", "sTitle": "Customers", "sType": "numeric"}
            ]
        });
