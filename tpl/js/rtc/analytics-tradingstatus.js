...
function getEmployeesForRetention() {
    ...
    apiRequestBrand('getLeverateEmployeesForRetention', $('#parameters-form').serialize(), '', true, capitalBrandName, function (data) {
...
function getUserData() {
    apiRequestBrand('getUserData', '', '', true, 'RTCGreg', function (data) {
        userData = data;
    });
}
function getLeadStatus() {
    $("#leadStatus").find('option').remove();
...
$(".list-group-item").not('.total').hover(function () {
    console.log("%cHover", "color: #f00;font-size:16px;");
    if ($(this).hasClass('available')) {
        $('.list-group-item.available').addClass('hover');
        $('.list-group-item').not('.available').removeClass('hover');
    } else {
        $('.list-group-item.available').removeClass('hover');
        $('.list-group-item').not('.available,.total').addClass('hover');
    }
}, function () {
    $(".list-group-item").removeClass('hover');
});
...
$(".list-group-item").focus(function () {
    ...
    if ($(this).hasClass('available')) {
        ...
        $('.list-group-item.available').addClass('focus').animate({opacity: 1}, 500);
        $('.list-group-item').not('.available,.total').animate({opacity: 0.25}, 500);
        ...
        $('.list-group-item.total').addClass('focus');
        $('#total_rab').animate({opacity: 0}, 500, function () {
        ...
        $('.list-group-item').not('.available,.total').addClass('focus').animate({opacity: 1}, 500);
        $('.list-group-item.available').animate({opacity: 0.25}, 500);
        $('#total_rab').animate({opacity: 0}, 500, function () {
            $(this).html('$' + total.toLocaleString());
            $(this).animate({opacity: 1}, 500);
        });
    ...
    $('.list-group-item').removeClass('focus').animate({opacity: 1}, 500);
    $('#total_rab').animate({opacity: 0}, 500, function () {
        $(this).html('$' + total_rab.toLocaleString());
        $(this).animate({opacity: 1}, 500);
    });
...
function getCountry() {
    $('#countries,#excludeCountries').find('option').remove();

    apiRequestBrand('getCountries', $('#range-form').serialize(), '', true, capitalBrandName, function (data) {
        jQuery.each(data, function () {
            $('#countries,#excludeCountries').append($('<option>', {value: this.countryName}).text(this.ISO + ' - ' + this.countryName));
        });
    });
}

function getTradingStatusList() {
    $('#tradeStatus').find('option').remove();
    apiRequestBrand('getTradingStatus', '', '', true, capitalBrandName, function (data) {
        populateTotalList(data);
        jQuery.each(data, function () {
            $('#tradeStatus')
                    .append($('<option>', {value: this.TradingStatusName}).text(this.TradingStatusName));
        });
    });
}

function populateTotalList(data) {
    totalList = data;
    var html_total = '<div class="list-group col-md-2">' +
            '<div class="list-group-item total"><h3 class="pull-right"><i class="fa fa-dollar"></i></h3>' +
            '<h4 class="list-group-item-heading" id="total_rab">-</h4>' +
            '<p class="list-group-item-text"><h5 id="total_rab_text">Total Real Account Balance</h5></p>' +
            '</div>';
    var section_run = '';
    var total = 0;
    var rows = 1;
    jQuery.each(data, function () {
        total++;
        var class_run = this.TypeName.toLowerCase();
        var id_path = (this.TradingStatusName.toLowerCase()).replace(/\s+/g, '');

        section_run = '<div class="list-group-item" ><div class=" ' + class_run + '"><h3 class="pull-right"><i class="fa fa-dollar"></i></h3>' +
                '<h4 class="list-group-item-heading" id="total_' + id_path + '">-</h4>' +
                '<p class="list-group-item-text"><h5 id="total_' + id_path + '_text">Total ' + this.TradingStatusName + '</h5></p>' +
                '</div></div>';

        if (total == data.length) {
            if (rows == 2) {
                html_total += '</div><div class="list-group col-md-2">';
            }
            html_total += section_run;
            html_total += '</div>';
        } else {
            if (rows != 2) {
                html_total += section_run;
    ...
    $(html_total).insertBefore("#customers_table_holder");
}
...
function changeTradeStatus() {
    $.ajax({
        url: "/apiRtc.php?cmd=changeTradeStatus",
        type: "POST",
...
function deleteTradeStatus() {
    $.ajax({
        url: "/apiRtc.php?cmd=deleteTradeStatus",
        type: "POST",
        data: $('#status-form').serialize(),
        dataType: "text",
        timeout: 60000000,
...
function deleteComment() {
    $.ajax({
        url: "/apiRtc.php?cmd=deleteComment",
        type: "POST",
        data: $('#comment-form').serialize(),
        dataType: "text",
        timeout: 60000000,
...
function getOpenPositionsForStatus() {
    apiRequestBrand('getOpenPositionsForStatus', $('#status-form').serialize(), $('#positions_table_holder'), true, capitalBrandName, function (data) {
...
function getOpenPositionsForTable() {
    apiRequestBrand('getOpenPositionsForStatus', $('#open-form').serialize(), $('#positions_table_holder'), true, capitalBrandName, function (data) {
        $('#positions_table').dataTable({
            "sDom": 'T<"clear">lfrtip',
            "oTableTools": {
                "sSwfPath": "/tpl/js/plugins/tabletools/swf/copy_csv_xls_pdf.swf",
...
function getPendingWithdrawals() {
    apiRequestBrand('getPendingWithdrawals', $('#pending-form').serialize(), $('#pending_table_holder'), true, capitalBrandName, function (data) {
...
function getNotesForStatus(customerId) {
    ...
    apiRequestBrand('getNotes', post_data, '#notes_table_holder', true, capitalBrandName, function (data) {
...
function getCommentsForStatus(customerId) {
    apiRequestBrand('getCommentsForStatus', 'customerId=' + customerId, $('#comments_table_holder'), true, capitalBrandName, function (data) {
...
function getSessionsForStatus() {
    ...
    apiRequestBrand('getSessionsForStatus', $('#session-form').serialize(), $('#session_table_holder'), true, capitalBrandName, function (data) {
...
function closeSession(id) {
    apiRequestBrand('closeTradeSessionForStatus', 'id=' + id, $('#session_table_holder'), true, capitalBrandName, function (data) {
...
function openTradeSessionForStatus() {
    apiRequestBrand('openTradeSessionForStatus', $('#openSession-form').serialize(), $('#session_table_holder'), true, capitalBrandName, function (data) {
...
function getTradeSessionPositions(post_data) {
    apiRequestBrand('getTradeSessionPositions', post_data, $('#sessionPositions_table_holder'), true, capitalBrandName, function (data) {
...
function getTradeStatus() {
    ...
    var brand = '';//capitalBrandName
    apiRequestBrand('getTradeStatus', $('#parameters-form').serialize(), $('#loader'), true, brand, function (data) {
        for (var i = 0; i < data.length; i++) {
