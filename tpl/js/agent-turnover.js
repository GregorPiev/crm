...
Date.prototype.format = function(format) //author: meizz
{
  ...
  function  getUserSpotId(){
  	apiRequest('getUserSpotId',$('#range-form').serialize(),'#portfolio_table_holder',function(spotId){	
  		if (spotId==0) {
  			getEmployees();
			$(".employee").show();
  			$(".desk").show();
  			...
  			$(".employee").remove();
  			$(".desk").remove();
  			$("#range-form").append('<input type="hidden" name="employee" value="'+spotId+'"/>');
  			$("#summery-form").append('<input type="hidden" name="employee" value="'+spotId+'"/>');
  		}
