...
switch ($_GET['cmd']) {        
        ...
        case 'getFaildDeposits':            
             if (!isset($_POST["dpStart"]) || !isset($_POST["dpEnd"]) || !isset($_POST["ftd"]))    
                die('{"error":"Missing arguments"}');
                
                $campaigns =!isset($_POST["campaign"])? '0':$_POST["campaign"];
                $employees =!isset($_POST["employee"])? '0':$_POST["employee"];             
                
                echo json_encode(getFaildDeposits($_POST["dpStart"], $_POST["dpEnd"],$campaigns, $_POST["desk"],  $employees, $_POST["ftd"]));
...
function getFaildDeposits($dpStart,$dpEnd,$campaignId, $desk,$employeeId, $ftdId){    
	$leverateConnect = new leverateDB_Connection($GLOBALS['db_host'],$GLOBALS['leverate_CRM']);               	
        
                $sql = "SELECT accounts.[AccountId] AS AccountId,
		        ...
		        AND CreatedOn BETWEEN '$dpStart 00:00:00' AND '$dpEnd 23:59:59' 
                                        ...
                                        GROUP BY CustomerId) AS dLog2 ON dLog2.CustomerId = dLog.CustomerId AND dLog2.maxDate = dLog.CreatedOn
		       ...
		       LEFT JOIN (SELECT  previous.lv_accountid AS accountId, MIN(previous.Lv_ApprovedOn) AS date 
                ...
                $sql .="  Where 1=1  And monetary.Lv_name = 'Deposit'  ";
                 if($campaignId!='0'){
...
function getByUserName($username){      
        ...
        $user = $MySqlDriverDB->fetchAll($query);
           if (!empty(array_filter($user)))  return $user[0];
            return false; 
}
