...
$_SESSION['userId'] = $_POST["user_id"];
	...
	if (!$country || $country=='') {
		$country = ($_SESSION["pre_post"]['Country_iso'] && $_SESSION["pre_post"]['Country_iso']!='') ? $_SESSION["pre_post"]['Country_iso'] : 'uk';
		$_POST['state'] = 'NA';
	} else {
		if($_POST['state']==''){
			switch ($country) {
...
<div class="form_container">
<form action="fibonatixrequest" METHOD="POST">
	...
	<input type="hidden" name="city" value="<?php echo ($_POST['City'] ? $_POST['City'] : 'NA' ) ?>" />
...
#process{
  width: 100px;
  height: 70px;
  background: #333 url('../../tpl/img/loaders/loading_mask.gif') center 12px no-repeat;
  border-radius: 3px;
