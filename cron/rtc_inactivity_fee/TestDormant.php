...
class DormantHelper {
	...
	public function getInactiveAccounts(){
		$sql = "SELECT accounts.AccountId accountGUID,
				...
				--	  AND accounts.AccountId IN ('8EE10DA6-8B43-4043-B4AE-0610C99557FB','49886AA8-3718-452A-802B-3001D01585F7')	            
	            ORDER BY accounts.AccountId";
          
         $result =  $this->leverateConnect->fetchAll($sql,array('accountGUID','tpAccountGUID'));
         var_dump($result);
         return $result;
	}
	...
	public function getInactiveTpAccounts($tpAccounts){
      ...
      //  return array(array('tpAccount'=>'837171'),array('tpAccount'=>'847089'),array('tpAccount'=>'847094'),array('tpAccount'=>'838876'));            
	 }
     ...
     public function tpAccountInactivityFees($tpAccountChanges){
     	foreach($tpAccountChanges as $key=>$tpAccountChange){
     		$sql = "INSERT INTO {$this->inventiva_CRM_DB}.dbo.TmpDormantCustomers (accountId,tpAccountId,tpAccount) VALUES ('{$tpAccountChange['accountGUID']}','{$tpAccountChange['tpAccountGUID']}','{$tpAccountChange['tpAccount']}')";
			$this->leverateConnect->exec($sql);
