...
switch ($_GET['cmd']) {
    ...
    case 'addCommissionBonus':
        if (!isset($_POST["bonusType"]) || !isset($_POST["bonusDate"]) || !isset($_POST["bonusEmployee"]) || !isset($_POST["bonusCurrency"]) || !isset($_POST["bonusAmount"]) || !isset($_POST["bonusPaymentMethod"]) || !isset($_POST["bonusReason"])
        )
            die('{"error":"Missing arguments"}');
    ...
    case 'getAgentPortfolio':
        if (!isset($_POST['employee']) || !isset($_POST['fdd']) || !isset($_POST['withdrawal']) || !isset($_POST['redeposit']) || !isset($_POST['totalDeposits']))
            die('{"error":"Missing arguments"}');
        ...
        echo json_encode(getAgentPortfolio($_POST['employee'], $_POST['fdd'], $countries, $leadStatus, $accountStatus, $_POST['withdrawal'], $_POST['redeposit'], $_POST['totalDeposits']));
        exit();
    ...
    case 'addNewCommunication':
        if (!isset($_POST['customer']) || !isset($_POST['templateId']) || !isset($_POST['title']) || !isset($_POST['description']) || !isset($_POST['leadStatus']))
            die('{"error":"Missing arguments"}');
        echo json_encode(addNewCommunication($_POST['customer'], $_POST['templateId'], $_POST['title'], $_POST['description'], $_POST['leadStatus']));
        exit();
    ...
    case 'getTradingStatus':
        echo json_encode(getTradingStatus());
        exit();
...
function getLeverateCustomer($customerId) {
    ...
    $sql = "SELECT accounts.AccountId AS Id,
			     ...
			     --  currencies.ISOCurrencyCode AS Currency,
			       ...
			       CASE WHEN accounts.Lv_SuppliedNecessaryDocuments = 1
			            THEN 'Yes'
			            ELSE 'No'
			       END AS Verification
    ...
    $customer[0]['add_deposit'] = $_SESSION['userdata']['per_' . $GLOBALS['brandName'] . '_deposit'];
...
function getCustomerCommunications($customerId) {
    ...
    $sql = "SELECT accounts.AccountId,
             ...
             (SELECT Value FROM StringMapBase ls
              WHERE ls.AttributeName = 'lv_leadstatus'
                     AND ls.ObjectTypeCode = objects.ObjectTypeCode
                     AND ls.AttributeValue = CommunicationLog.OldLeadStatus
					 AND ls.LangId = 1033) AS OldLeadStatus,
             (SELECT Value FROM StringMapBase ls
              WHERE ls.AttributeName = 'lv_leadstatus'
                     AND ls.ObjectTypeCode = objects.ObjectTypeCode
                     AND ls.AttributeValue = CommunicationLog.NewLeadStatus
					 AND ls.LangId = 1033) AS NewLeadStatus
            ...
            LEFT JOIN {$GLOBALS['inventiva_CRM_DB']}.dbo.CommunicationLog as CommunicationLog ON CommunicationLog.AnnotationId = communications.AnnotationId
...
function getCustomerDeposits($customerId) {
    ...
    $sql = "SELECT tpAccounts.Lv_name AS tpAccount,
	               ...
	               CASE WHEN deposits.new_Is3D = 1
	                     THEN 'Yes'
	                     ELSE 'No'
	                END AS is3D,
	                deposits.new_processor AS processor
            FROM Lv_monetarytransactionBase AS deposits
            LEFT JOIN (SELECT BaseTableName, ObjectTypeCode
                       FROM MetadataSchema.Entity
...
function getCustomerDepositLog($customerId) {
    ...
    $sql = "SELECT CONVERT(DATETIME2(0),dLog.CreatedOn) AS date,
                  ...
                  Tools.dbo.customSplit(SUBSTRING(dLog.Description,CHARINDEX('card last 4 digits',dLog.Description),CHARINDEX('card expiry month',dLog.Description)-CHARINDEX('card last 4 digits',dLog.Description)),'->',2) AS cardNum,
				  Tools.dbo.customSplit(SUBSTRING(dLog.Description,CHARINDEX('PROCESSOR',dLog.Description),LEN(dLog.Description)-CHARINDEX('card last 4 digits',dLog.Description)),'=',2) AS processor,
				  ...
				  Tools.dbo.customSplit(SUBSTRING(dLog.Description,CHARINDEX('IP address',dLog.Description),(CASE WHEN CHARINDEX('PROCESSOR',dLog.Description) <> 0 THEN CHARINDEX('PROCESSOR',dLog.Description) ELSE LEN(dLog.Description) END)-CHARINDEX('IP address',dLog.Description)),'->',2) AS ipAddress
...
function getLeverateEmployeesForRetention($desk) {
    ...
    return $leverateConnect->fetchAll($sql, array('userId', 'BusinessUnitId'));
}
...
function getLeverateTransactionsForCommission($dpStart, $dpEnd, $desk, $employee) {
    ...
    $sql = "SELECT deposits.Lv_monetarytransactionId AS depositId,
                 ...
                 customers.Lv_SuppliedNecessaryDocuments,
...
function getAllCommissionBonuses($dpStart, $dpEnd, $desk, $employee) {
    ...
    $sql = "SELECT CC.BusinessUnitId as id,
               Currency.CurrencyCode
            FROM dbo.CommissionCurrency as CC
            LEFT JOIN dbo.Currency as Currency ON CC.CurrencyId = Currency.CurrencyId";
    $data['commission_currencies'] = $leverateConnect->fetchAll($sql, array('id'));
...
function getLeverateTransactions($dpStart, $dpEnd, $campaigns, $desk, $employee, $current) {
    ...
    $sql = " SELECT transactions.Lv_monetarytransactionId AS transactionId,
	               ...
	               -- transactions.lv_TradingPlatformTransactionId AS tpTransactionId,
	                ...
	                businessUnit.Name AS businessUnit,
	                CASE WHEN transactions.new_Is3D = 1
	                     THEN 'Yes'
	                     ELSE 'No'
	                END AS is3D,
	                transactions.new_processor AS processor,
	                transactions.new_psptransactionid AS PSPTransactionId,
	                transactions.new_promocode AS promoCode
             ...
             LEFT JOIN SystemUserBase AS transactionEmployees ON transactionEmployees.SystemUserId = ISNULL(changeEmployee.OwnerId,transactions.lean_TransactionOwner)
...
function addNewCommunication($customer, $templateId, $title, $description, $leadStatus) {
    require_once "inc/leverate/leverateApi.php";
    ...
    if ($result->Code == 'Success') {
        ...
        for ($count = 0; $count < 10; $count++) {
            ...
            if ($updatedLastNote != $lastNote) {
                $newAnnotationId = $updatedLastNote[0]['AnnotationId'];
                break;
            }
            sleep(1);
        }
        $newAnnotationId = $newAnnotationId ? "'$newAnnotationId'" : "NULL";
        $templateId = $templateId ? intval($templateId) : "NULL";
        $preLeadStatus = $preLeadStatus ? $preLeadStatus : 0;
        $leadStatus = $leadStatus ? intval($leadStatus) : 0;
        $sql = "INSERT INTO {$GLOBALS['inventiva_CRM_DB']}.dbo.CommunicationLog (AnnotationId,CommunicationTemplateId,OldLeadStatus,NewLeadStatus)
                                       VALUES ($newAnnotationId,$templateId,'$preLeadStatus','$leadStatus')";
        $leverateConnect->exec($sql);
    }
    return $result;
}
...
function getRealPnl($startDate, $endDate, $desk, $employee, $onlyClose) {
    ...
    if (!empty($customers)) {
        ...
        $sql = "SELECT accounts.LOGIN,
                       ...
                       ROUND(" . ($endDate == $today ? "users.EQUITY - users.BALANCE - users.CREDIT" : "accounts.end_Open_Pnl") . ",2) AS end_Open_Pnl,
					   ROUND(accounts.end_Closed_Pnl " . ($endDate == $today ? "+ IFNULL(closed_positions.profit,0) + users.EQUITY - users.BALANCE - users.CREDIT" : "+ accounts.end_Open_Pnl") . ",2) AS end_Pnl,
					   ROUND(" . ($endDate == $today ? "users.BALANCE" : "IFNULL((SELECT BALANCE
					                                          FROM {$GLOBALS['brandName']}_daily_view
					                                          WHERE LOGIN = accounts.LOGIN
					                                                AND TIME ='$endDate 23:59:59'
					                                          ),0)") . ",2) AS balance,
					   ROUND(" . ($endDate == $today ? "users.EQUITY" : "IFNULL((SELECT EQUITY
					                                          FROM {$GLOBALS['brandName']}_daily_view
					                                          WHERE LOGIN = accounts.LOGIN
					                                                AND TIME ='$endDate 23:59:59'
					                                          ),0)") . ",2) AS equity
...
function getFailedDeposits($dpStart, $dpEnd, $campaigns, $desk, $employee, $ftd) {
    ...
    $sql = "SELECT accounts.AccountId,
            ...
            . ($desk ? " AND employee.BusinessUnitId = '$desk' " : "") .
            ($employee ? " AND employee.SystemUserId IN ('" . implode("','", $employee) . "') " : "") . "
    ...
    $result = $leverateConnect->fetchAll($sql, array('AccountId'));
    $failed_deposits = array();
    foreach ($result as $key => $value) {
        if ($value['employee'] != '')
            $failed_deposits[] = $value;
    }
...
function getTemplateList() {
    ...
    $sql = "SELECT CommunicationTemplateId AS id ,
	               Name ,
	               Subject,
	               NoteText,
	               LeadStatus
...
function getTradingStatus() {
    ...
    $sql = "SELECT ts.TradingStatusId,
	               ts.TradingStatusName,
	               type.TypeName
	        FROM TradingStatus ts
	        LEFT JOIN TradingStatusType type ON type.TradingStatusTypeId = ts.TradingStatusTypeId
	       ";
    return $leverateConnect->fetchAll($sql);
}
?>